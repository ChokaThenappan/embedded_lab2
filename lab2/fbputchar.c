/*
 * fbputchar: Framebuffer character generator
 *
 * Assumes 32bpp
 *
 * References:
 *
 * http://web.njit.edu/all_topics/Prog_Lang_Docs/html/qt/emb-framebuffer-howto.html
 * http://www.diskohq.com/docu/api-reference/fb_8h-source.html
 */

#include "fbputchar.h"
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <sys/mman.h>
#include <sys/ioctl.h>
#include "map.h"

#include <linux/fb.h>

#define FBDEV "/dev/fb0"

// FONT_WIDTH and FONT_HEIGHT correspond to the number of pixels for each character (height is greater than the width of a character on screen)
#define FONT_WIDTH 8
#define FONT_HEIGHT 16
#define BITS_PER_PIXEL 32

struct fb_var_screeninfo fb_vinfo;
struct fb_fix_screeninfo fb_finfo;
unsigned char *framebuffer;
static unsigned char font[];

/*
 * Open the framebuffer to prepare it to be written to.  Returns 0 on success
 * or one of the FBOPEN_... return codes if something went wrong.
 */
int fbopen()
{

  
  // Open returns an integer value (file descriptor) that will be used in subsequent system calls to refer to the file (will be the lowest-numbered fd not open for the process)
  // The file/device is opened for Read and Write
  int fd = open(FBDEV, O_RDWR); /* Open the device */
  if (fd == -1) return FBOPEN_DEV;

  // Manipulate the device's parameters and control operating characteristics of the device
  // Device independent unchangeable info about fb device and video mode
  if (ioctl(fd, FBIOGET_FSCREENINFO, &fb_finfo)) /* Get fixed info about fb */
    return FBOPEN_FSCREENINFO;

  // Device independent changeable information about fb device and video mode
  if (ioctl(fd, FBIOGET_VSCREENINFO, &fb_vinfo)) /* Get varying info about fb */
    return FBOPEN_VSCREENINFO;

  // Struct contains generic infor of the fb device - one of them is bits per pixel (expected to be 32)
  if (fb_vinfo.bits_per_pixel != 32) return FBOPEN_BPP; /* Unexpected */

   
  // Creates a new mapping in the address space of the process -> the device will appear to the user corresponding to a set of addresses
  // Starting address: 0
  // smem_len: length of framebuffer memory (in bytes)
  // Offset: 0
  // Memory Protection: Read and Write
  // Mapping is shared between processes
  // Returns a pointer to the mapped area
  framebuffer = mmap(0, fb_finfo.smem_len, PROT_READ | PROT_WRITE,
		     MAP_SHARED, fd, 0);
  if (framebuffer == (unsigned char *)-1) return FBOPEN_MMAP;

  return 0;
}

/*
 * Draw the given character at the given row/column.
 * fbopen() must be called first.
 */
void fbputchar(char c, int row, int col)
{
  int x, y;

  // pixelp points to one of the elements of the array, based on the given character and the FONT_HEIGHT = 16
  // The character is given as an ASCII code -> pixelp points to the first element in the row 16*c (this row corresponds to the character)
  unsigned char pixels, *pixelp = font + FONT_HEIGHT * c;
  unsigned char mask;
  // xoffset, yoffset: offset from virtual to visible resolution
  // line_length: length of line in bytes
  // left points to the first memory address which corresponds to the first pixel of the row/column of screen we want to draw
  unsigned char *pixel, *left = framebuffer + (row * FONT_HEIGHT * 2 + fb_vinfo.yoffset) * fb_finfo.line_length + (col *   * 2 + fb_vinfo.xoffset) * BITS_PER_PIXEL / 8;


  for (y = 0 ; y < FONT_HEIGHT * 2 ; y++, left += fb_finfo.line_length) {
    pixels = *pixelp;				// Pixels variable has the first element of the respective row in font array
    pixel = left;				// Pixel variable points to the first byte of the pixel in memory
    mask = 0x80;
    for (x = 0 ; x < FONT_WIDTH ; x++) {	// In total 16 pixels are field in these 8 iterations (This is done to make the letters thicker)
      if (pixels & mask) {			// Filling the 4 bytes with the correct values
	pixel[0] = 255; /* Red */
        pixel[1] = 255; /* Green */
        pixel[2] = 255; /* Blue */
        pixel[3] = 0;
      } else {
	pixel[0] = 0;
        pixel[1] = 0;
        pixel[2] = 0;
        pixel[3] = 0;
      }
      pixel += 4;				// Jumping to the next pixel in memory (fb) and filling the 4 bytes
      if (pixels & mask) {
	pixel[0] = 255; /* Red */
        pixel[1] = 255; /* Green */
        pixel[2] = 255; /* Blue */
        pixel[3] = 0;
      } else {
	pixel[0] = 0;
        pixel[1] = 0;
        pixel[2] = 0;
        pixel[3] = 0;
      }
      pixel += 4;				// Jumping to the next pixel in memory (fb) and filling the 4 bytes
      mask >>= 1;				
    }
    if (y & 0x1) pixelp++;			// Move to the next element of font every 2 iterations of y (same element is used for 2 y)
  }
}

/*
 * Draw the given string at the given row/column.
 * String must fit on a single line: wrap-around is not handled.
 */
void fbputs(const char *s, int row, int col)
{
  char c;
  for (int i = 0; i < 64; i++) fbputchar(' ', row, i);  // Clear line before writing
  while ((c = *s++) != 0) fbputchar(c, row, col++);
}

/* 8 X 16 console font from /lib/kbd/consolefonts/lat0-16.psfu.gz

od --address-radix=n --width=16 -v -t x1 -j 4 -N 2048 lat0-16.psfu

*/

// Every row of the font array corresponds to one of the 128 ASCII characters
// This array shows how each character should be written in memory to appear correctly in screen
// Each element of the array has size 8 bits (FONT_WIDTH) -> 16 (FONT_HEIGHT) elements per character
static unsigned char font[] = {
  ,
};
